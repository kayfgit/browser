...
[] - command to yank current url
[] - be able to highlight stuff on command bar (shift, ctrl+shift+arrowkeys should work)
[] - allow copy and paste on command bar
[] - make :edit use the same mode as the tab was opened (currently if i open a tab using :research and type :edit it goes to :open <url> instead of :research <url>)
